...
boolean flagRedirect = ParamUtil.getBoolean(request, "flagRedirect", false);
%>
...
<c:choose>
	...
	<c:otherwise>
		...
		<div class="title-tinnoibat"> Bình chọn </div>
		...
		<aui:form action="<%= voteQuestionURL %>" method="post" name="fm">
			...
			<c:choose>
				<%-- <c:when test="<%= (!question.isExpired() && !hasVoted && PollsQuestionPermission.contains(permissionChecker, question, ActionKeys.ADD_VOTE)) && !flagRedirect %>"> --%>
				<c:when test="<%= !themeDisplay.isSignedIn() && !question.isExpired() && !PollsQuestionPermission.contains(permissionChecker, question, ActionKeys.ADD_VOTE) %>" >
				...
				<c:otherwise>
					<aui:fieldset>
						...
						<c:if test="<%= !question.isExpired() && !hasVoted && !flagRedirect %>">
							<aui:button type="submit" value="vote[action]" />
						</c:if>
						
						<c:if test="<%= !flagRedirect %>">
							<portlet:renderURL var="showResult" windowState="<%=LiferayWindowState.POP_UP.toString() %>">
								<portlet:param name="flagRedirect" value="true"/>
							</portlet:renderURL>
					
							<%
								String js = "viewPoolsResult('"+ showResult.toString() + "')";
							%>
							
							<aui:button type="button" value="view-pool-result" name="view-pool-result" onclick="<%=js %>"/>
						</c:if>
					</aui:fieldset>
				</c:otherwise>
			</c:choose>
...
<aui:script use="aui-base">
	Liferay.provide(
			...
			'viewPoolsResult',
			function(url){
				var title="<liferay-ui:message key='view-pool-result'/>";
				var dialogWidow = {
					dialog: {
						constrain: true,
