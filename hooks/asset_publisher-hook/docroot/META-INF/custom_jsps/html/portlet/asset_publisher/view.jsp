...
PortletURL rowURL1 = PortletURLFactoryUtil.create(request, "101_INSTANCE_iVSSyRXdH68n", selectlayoutId, PortletRequest.RENDER_PHASE);
%>


<c:if test="<%=Validator.isNotNull(portletTitle1) %>" >
		<h4 class="news-header-title">
			<c:choose>
				<c:when test="<%= Validator.isNotNull(rowURL1) %>">
					<a href="<%= rowURL1 %>" title="<%= HtmlUtil.escape(portletTitle1) %>"><%= HtmlUtil.escape(portletTitle1) %></a>
				</c:when>
				<c:otherwise>
					<span><%= HtmlUtil.escape(portletTitle1) %></span>
				</c:otherwise>
			</c:choose>
		</h4>
	</c:if>
