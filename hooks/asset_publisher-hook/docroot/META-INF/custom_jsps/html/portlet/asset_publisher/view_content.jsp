...
if(Validator.isNotNull(assetRendererFactory)){
	if (Validator.isNull(assetEntryId)) {
		if(assetRendererFactory.getClassName().equals(JournalArticle.class.getName())) {
			assetRenderer = _getAssetRenderer(groupId, urlTitle);
...
if(Validator.isNotNull(assetRenderer)){
	...
	if (!assetEntry.isVisible() &&
	 ...
	 viewURLMessage = StringPool.BLANK;
...
private static AssetRenderer _getAssetRenderer(long groupId, String urlTitle) {

	AssetRenderer assetRenderer = null;
	
	try {
		JournalArticle article =
			JournalArticleLocalServiceUtil.getDisplayArticleByUrlTitle(
				groupId, urlTitle);

		assetRenderer = new JournalArticleAssetRenderer(article);
	}catch(Exception e) {
