				...
				<div class="lfr-pagination-config">
					<div class="lfr-pagination-page-selector">
						<c:choose>
							...
							<c:otherwise>
								...
								<%
								String suffix = LanguageUtil.get(pageContext, "Trong") + StringPool.SPACE + numberFormat.format(pages);
								...
								<liferay-ui:icon-menu
									cssClass="current-page-menu custom-paging"
									direction="down"
									icon=""
									message='<%= StringPool.SPACE + cur + StringPool.SPACE %>'
					...
					<div class="lfr-pagination-delta-selector">
						<div style="margin-right: 5px">
							<liferay-ui:message key="cp-page" />
						</div>
     ...
     if (total > delta) {
          ...
          for (int i = 1; i <= totalPages; i++) {
             if (totalPages > 5) {
            	 if (i == cur) {
                     %>
                     <li class="active"><a href="<%= url + namespace + curParam + "=" + i + urlAnchor %>"><%=String.valueOf(i) %><span class="sr-only"></span></a></li>
                     <%
                 } else if ((i == (cur - 1)) || (i == (cur + 1))) {
                 %>
                     <li><a href="<%= url + namespace + curParam + "=" + i + urlAnchor %>"><%=String.valueOf(i) %></a></li>
                 <%
                 } 
             } else {
            	 if (i == cur) {
