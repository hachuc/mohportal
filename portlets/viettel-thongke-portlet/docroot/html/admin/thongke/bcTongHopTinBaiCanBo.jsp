...
<portlet:resourceURL var="callMyUrl">
     ...
     <liferay-portlet:renderURL varImpl="baoCaoTongHopTinBaiCanBoURL" var="">
	    <liferay-portlet:param name="jspPage" value="/html/admin/thongke/view.jsp"/>
    ...
    <form   method="post" action="<%=callMyUrl%>">
       ...
       <aui:row> 
           <aui:column>
           <%if(!keywords.equals("")){ %>
           <label><%= LanguageUtil.get(locale, "keywords")+": "+keywords %> </label>
           <%} %>
           <aui:input name="keywords" label="keywords" type="hidden" value="<%=keywords %>" ></aui:input>
	       </aui:column>
		   <aui:column>
		   <%if(!fromDate.equals("")){ %>
		   <label><%= LanguageUtil.get(locale, "from-date")+": "+fromDate %> </label>
	       <%} %>
	       <aui:input name="fromDate" label="from-date" type="hidden" value="<%=fromDate %>" ></aui:input>
	       </aui:column>
		   <aui:column>
		   <%if(!toDate.equals("")){ %>
		    <label><%= LanguageUtil.get(locale, "to-date")+": "+toDate %> </label>
	       <%} %>
		...
		<aui:row> 
		   <aui:column>
				...
				<aui:input name="fileName" label="" type="hidden" value="tong_hop_tin_bai_can_bo.xlsx" ></aui:input>
   ...
   <script>
        document.getElementById("headerReport").innerHTML='<%= LanguageUtil.get(locale, "tong-hop-tin-bai-can-bo") %>';
   </script>