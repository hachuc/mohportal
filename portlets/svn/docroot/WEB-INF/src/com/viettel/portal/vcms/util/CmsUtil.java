...
import com.viettel.portal.banner.util.BannerConstants;
import com.viettel.portal.banner.util.PortletPropsValues;*/
...
public class CmsUtil {
	...
	/*public static boolean validateImageThumbnail(String imageFileName,long imageFileSize,
			PortletPreferences preferences)throws BannerFileTypeException, BannerFileSizeException{
		
		boolean flag = false;
		
		String[] validImageExtensions = preferences.getValues("imageThumbnailExtensions",
				BannerConstants.DEFAULT_IMAGE_THUMBNAIL_EXTENSIONS);
		...
		if(!flag){
			...
			throw new BannerFileTypeException();
		}
		
		long imageThumbnailMaxSize = GetterUtil.getLong(preferences.getValue("imageThumbnailMaxSize",
				String.valueOf(BannerConstants.DEFAULT_IMAGE_THUMBNAIL_MAX_SIZE))) * BannerConstants.FILE_SIZE_COEFFICENT;
		...
		if ((imageThumbnailMaxSize > 0)
			...
			throw new BannerFileSizeException();
		}
		
		return flag;
	}
	
	public static boolean validateBannerFile(String videoFileName, byte[] videoFileBytes) 
			throws BannerFileTypeException, BannerFileSizeException {
