...
public class VanBanPhapQuyFinderImpl extends BasePersistenceImpl<VanBanPhapQuy>
	...
	private List<VanBanPhapQuy> findBy_K(String[] keywords, long groupId,
		...
		try {
			...
			System.err.println("REVOTECH PORTAL: " + sql);
...
*/
	...
	public int timKiemVanBanPhapQuyCount(String keyword,
		...
		try {
			...
			String sql = " FROM portal_vanban "
					...
					;
			params.add(groupId);
			_log.info("REVOTECH PORTAL groupId: " + groupId);
			if(keyword == null){
				keyword = "";
			}
			else{
			...
			_log.info("REVOTECH PORTAL keyword: " + "%" + keyword + "%");
			params.add("%" + keyword + "%");
			params.add("%" + keyword + "%");
			params.add("%" + keyword + "%");
			if (!ArrayUtil.isEmpty(coQuanBanHanhIds)) {
			...
			if(status != null) {
				sql += " AND (status = ?) ";
				params.add(status);
				_log.info("REVOTECH PORTAL status: " + status);
			}
			if(startDateObj != null) {
				sql += " AND (ngaybanhanh >= ?) ";
				params.add(startDateObj);
				_log.info("REVOTECH PORTAL startDateObj: " + startDateObj);
			}
			if(endDateObj != null) {
				sql += " AND (ngaybanhanh <= ?) ";
				...
				_log.info("REVOTECH PORTAL endDateObj: " + endDateObj);
			}
			SQLQuery q = session.createSQLQuery(" SELECT count(*) as TOTAL  " + sql);
			_log.info("REVOTECH PORTAL sql: " + sql);
	...
	public VanBanPhapQuyGrid timKiemVanBanPhapQuy(String keyword,
		...
		try {
			...
			_log.info("REVOTECH PORTAL total: " + total);
