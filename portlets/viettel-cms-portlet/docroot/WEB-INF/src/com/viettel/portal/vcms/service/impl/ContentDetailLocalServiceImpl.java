...
public class ContentDetailLocalServiceImpl extends ContentDetailLocalServiceBaseImpl {
	 ...
	 */
	public void deleteByVersions (long groupId, long contentId, int[] versions) throws SystemException, PortalException{
		List<ContentDetail> lstResults = ContentDetailFinderUtil.findByVersion(groupId, contentId, versions);
		if(lstResults!=null){
			for(ContentDetail detail : lstResults){
				ContentDetailUtil.remove(detail.getAutoId());
			}
		}
	}
	public List<ContentDetail> findByVersions (long groupId, long contentId, int[] versions) throws SystemException, PortalException{
		return ContentDetailFinderUtil.findByVersion(groupId, contentId, versions);
	}
