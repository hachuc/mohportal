...
public class MigratePortlet extends MVCPortlet {
	...
	public void MigrateContent(ActionRequest actionRequest, ActionResponse actionResponse) throws PortalException, SystemException{
		try {
			...
			boolean isMgrComment = ParamUtil.getBoolean(actionRequest, "migratecComment", false);
			...
			for(Legal_Zone zone : lstZone){
				...
				int sizeOfContents = Legal_ContentLocalServiceUtil.getCountLegalContentByZoneId(zone.getID());
				if (sizeOfContents == 0)
					continue;
				int oddPage = sizeOfContents / pageSize;
				int divNews = sizeOfContents % pageSize;
				if(divNews == 0)
					totalPage = oddPage;
				else
					totalPage = oddPage + 1;
				...
				List<Legal_Content> lstLegalContents = Legal_ContentLocalServiceUtil.getLegalContents(zone.getID(), start, end);
					if(!lstLegalContents.isEmpty()){
						if(start > 0) start = start + pageSize + 1;
						...
						for(Legal_Content lcontent : lstLegalContents){
							...
							_log.info("Migrated -->" + lcontent.getHeadline());
						}
					}
				}
