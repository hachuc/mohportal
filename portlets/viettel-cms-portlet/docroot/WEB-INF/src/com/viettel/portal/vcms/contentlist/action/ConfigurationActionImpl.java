...
public void processAction(PortletConfig portletConfig,
	...
	try {
		...
		int[] arrLayoutIndex = new int[]{1,2,4,8,16,32,64,128,256,512, 1000, 1024,2048,4096,8192};
		boolean showForAuthor = ParamUtil.getBoolean(actionRequest, "showForAuthor");
		boolean showOnlyOwnContent = ParamUtil.getBoolean(actionRequest, "showOnlyOwnContent", false);
		for(int i = 0; i < arrLayoutIndex.length; i++){
			String linkToPage = ParamUtil.getString(actionRequest, "linkToPage" + arrLayoutIndex[i]);
			preferences.setValue("linkToPage" + arrLayoutIndex[i], linkToPage);
		}
		...
		preferences.setValue("showOnlyOwnContent", String.valueOf(showOnlyOwnContent));
