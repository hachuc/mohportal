...
<div class="feature">
		...
		%>
		<article class="story story--primary clearfix">
			<a href="<%=viewURL%>" title="<%= HtmlUtil.escape(title) %>" class="story__thumb"><img src="<%= smallImageUrl %>" alt="" class="loading" data-was-processed="true"></a>
			<h2><a href="<%=viewURL%>" title="<%= HtmlUtil.escapeAttribute(title) %>" class="story__title"><%= HtmlUtil.escape(title) %></a></h2>
			<div class="meta">
				<c:if test='<%= showPublishDate %>'>
					<time class="timebox done" rel="1543401120000"><%= publishDate %></time>
				</c:if>
				<c:if test='<%= showViewCount %>'>
					<span><i class="icon-bubble"></i><%= String.valueOf(viewCount) %></span>
				</c:if>
			</div>
		</article>
		<%
