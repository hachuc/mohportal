	...
	<aui:form action="<%=searchURL%>" method="get" name="fm"
		...
		<liferay-ui:panel-container cssClass="basic-info"
			...
			<liferay-ui:panel collapsible="<%=true%>" extended="<%=true%>"
				...
				title='<%="page-title-v" + roleVersion + "s"
								+ stateValue%>'>
		...
		<aui:fieldset>
			...
			/>
				...
				<liferay-ui:search-container-row
					...
					keyProperty="autoId" modelVar="content">
					...
					<liferay-ui:search-container-column-text name="STT" cssClass="align-center"
						value="<%=String.valueOf(index + 1)%>" href="<%=rowURL%>" />

					<liferay-ui:search-container-column-text name="search-by-title"
						href="<%=rowURL%>" value="<%=HtmlUtil.escape(content.getTitle())%>">

					</liferay-ui:search-container-column-text>

					<liferay-ui:search-container-column-text href="<%=rowURL%>"
						name="editor" value="<%=HtmlUtil.escape(content.getEditedByUserName())%>" />
					<liferay-ui:search-container-column-text href="<%=rowURL%>"
						name="create-date" value="<%=dateFormat.format(content.getCreateDate())%>" />
					<liferay-ui:search-container-column-text name="option" cssClass="align-center double-icon">
							
							<portlet:actionURL var="rewriteContentURL" name="rewriteContent">
                                <portlet:param name="autoId" value="<%=String.valueOf(content.getAutoId())%>" />
                                <portlet:param name="contentId" value="<%=String.valueOf(content.getContentId())%>" />
							...
							<portlet:actionURL var="deleteContentURL" name="deleteContent">
								<portlet:param name="autoId" value="<%=String.valueOf(content.getAutoId())%>" />
							...
							<a href="<%= deleteContentURL.toString() %>" title="delete" onclick="return confirm('Bạn muốn xóa bài viết này?');"><i class="glyphicon icon-trash"></i></a>
