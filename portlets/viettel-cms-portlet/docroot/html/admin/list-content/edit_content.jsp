...
<%@ taglib uri="http://liferay.com/tld/ui" prefix="liferay-ui"%>
<%
	System.out.println("edit_content");
...
<aui:form name="fm" method="post" action="" cssClass="form-horizontal" onSubmit='<%= taglibOnSubmit %>' enctype="multipart/form-data">
	...
	<c:choose>
		<c:when test='<%= content!=null && (content.getVersion()==1 || content.getVersion() == 2 && (content.getStatus() == 4)) && content.getCreatedByUserId()!=userId%>'>
			<% editStatus = false; %>
		...
		<c:when test='<%= content!=null && content.getVersion()==3 && (content.getReviewedByUserId() > 0 && content.getReviewedByUserId()!=userId)%>'>
			<div class="alert alert-danger">
		...
		<%-- <c:when test='<%= content!=null && content.getVersion()==2  && userId!=content.getCreatedByUserId() %>'>
			<div class="alert alert-danger">
				<liferay-ui:message key="content-is-being-created-by" /> : <%= content.getCreatedByUserId() %>
			</div>
			<% editStatus = false; %>
		</c:when> --%>
	...
	<div class="form-group">
		...
		<div class="col-sm-7 col-xs-12">
			<aui:select name="newsType" label="">
				...
				%>
				<c:if test='<%= editStatus && cat.getContenTypeId() == content.getNewsType() %>'>
	...
	<c:if test="<%= (content.getVersion() == 4 && content.getStatus()==8192)%>">
		<div class="form-group">
			<label class="label-control col-md-2 col-xs-12">
				<liferay-ui:message key="phan-hoi" />
			</label>
			<div class="col-sm-7 col-xs-12">
				<input type="checkbox" name="phanHoi" id="phanHoi">
				<label for="phanHoi" class="label-control">Có</label>
			</div>
			<div class="col-sm-3 hidden-xs hidden-sm">
			</div>
		</div>	
	...
	<div class="form-group">
		...
		<div class="col-sm-10 col-xs-12 pl0">
            ...
            <c:if test="<%= (content.getStatus() != 8192) %>">
            	<a class="btn btn-small" href="<%=content.getVersion()<4 ? forwardActionURL.toString():publishActionURL.toString() %>" role="button" ><i class="glyphicon icon-circle-arrow-right"></i> <liferay-ui:message key='<%= content.getVersion() < 4 ?"gui-bai-viet-nay":"dang-bai-viet-nay" %>' /></a>
            </c:if>
		</div>
	</div>
