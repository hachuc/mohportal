package com.viettel.portal.newspublisher.util;
...
public class NewsPublisherUtil {
		...
		try {
			...
			if (articleDisplay.isSmallImage()) {
				if (Validator.isNotNull(articleDisplay.getSmallImageURL())) {
					...
					smallImagePath = themeDisplay.getPathImage() + "/journal/article?img_id=" + articleDisplay.getSmallImageId() + "&t=" + WebServerServletTokenUtil.getToken(articleDisplay.getSmallImageId());
				}
			}
		} catch (Exception e) {
			_log.error(e);
	...
	/**
	 * @todo TAO LINK XEM CHI TIET TIN BAI DUA VAO PAGELAYOUTID (plId) DUOC CAU
	 *       HINH HOAC SET TRONG THUOC TINH "link-to-page" CUA CHUYEN MUC CHUA
	 *       TIN BAI
	 * @author TrungNT
	 * @date Nov 17, 2014
	 * @return String
	 ...
	 * @return
	 */
	public static String getViewContentURL(HttpServletRequest request, AssetEntry assetEntry, String layoutUuid, long groupId) {
	...
	/**
	 * Ham get link chi tiet bai viet <br>
	 * Su dung trong truong hop portlet AssetPublisher de dang instance=false
	 * 
	 * @param request
	 * @param assetEntry
	 * @param layout
	 ...
	 * @return
	 */
	public static String getViewContentURL(HttpServletRequest request, AssetEntry assetEntry, Layout layout) {
		...
		if (assetEntry != null && layout != null) {
			try {
				...
				if (assetPublisherPortletId != null) {
					...
					if (PortletPropsValues.ASSET_PUBLISHER_INSTANCE) {
						viewFullContentURL.setParameter("type", assetEntry.getAssetRendererFactory().getType());
					}
					...
					if (themeDisplay.getScopeGroupId() != groupId && groupId > 0) {
						viewFullContentURL.setParameter("groupId", String.valueOf(groupId));
					}

					if (Validator.isNotNull(assetEntry.getAssetRenderer().getUrlTitle())) {
						viewFullContentURL.setParameter("urlTitle", assetEntry.getAssetRenderer().getUrlTitle());
					}
				}
