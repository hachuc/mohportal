...
public class PhotoFileUtil {
	public static String getPhotoThumbnailURL(ThemeDisplay themeDisplay,FileEntry thumbnailEntry){
	...
	public static boolean validateImageThumbnail(String imageFileName,long imageFileSize,
			PortletPreferences preferences)throws PhotoFileTypeException, PhotoFileSizeException{
		
		boolean flag = false;
		
		String[] validImageExtensions = preferences.getValues("imageThumbnailExtensions",
				PhotoConstants.DEFAULT_IMAGE_THUMBNAIL_EXTENSIONS);
		...
		if(!flag){
			...
			throw new PhotoFileTypeException();
		}
		
		long imageThumbnailMaxSize = GetterUtil.getLong(preferences.getValue("imageThumbnailMaxSize",
				String.valueOf(PhotoConstants.DEFAULT_IMAGE_THUMBNAIL_MAX_SIZE))) * PhotoConstants.FILE_SIZE_COEFFICENT;
		
		//Validate upload file size
		if ((imageThumbnailMaxSize > 0) && (imageFileSize > imageThumbnailMaxSize)) {

			//flag = false;
			
			throw new PhotoFileSizeException();
		}
		
		return flag;
	}
	
	public static boolean validatePhotoFile(String videoFileName, byte[] videoFileBytes) 
			throws PhotoFileTypeException, PhotoFileSizeException {
		
		boolean flag = false;

		String[] validFileExtensions = PortletPropsValues.BANNER_FILE_EXTENSIONS;
		...
		long fileMaxSize = PortletPropsValues.BANNER_FILE_MAX_SIZE;
	...
	private static Log _log = LogFactory.getLog(PhotoFileUtil.class);
}
