...
<%
	String  tabs1 = ParamUtil.getString(request,"tabs1","Feedback");
	//String tabNames = "FAQEntries,FAQCategories,FAQDepartments,Feedback,Settings";
	String tabNames = "Feedback";
%>
...
<liferay-ui:tabs names="<%=tabNames %>" url="<%=legalFAQManagementURL.toString() %>">
	...
	<c:if test='<%=tabs1.equals("FAQDepartments") %>'>
	    <liferay-util:include page="/html/legal_faq_management_portlet/faq_category/category_survey_manegement.jsp" servletContext="<%=this.getServletContext() %>"/>
	</c:if>
	<c:if test='<%=tabs1.equals("Feedback") %>'>
	    <liferay-util:include page="/html/legal_faq_management_portlet/feedback/feedback_manegement.jsp" servletContext="<%=this.getServletContext() %>"/>
	</c:if>
